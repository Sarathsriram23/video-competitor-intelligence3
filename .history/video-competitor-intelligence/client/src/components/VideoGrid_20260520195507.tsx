                  ...
                  <div className="p-3 flex flex-col flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2 text-[10px]">
                      ...
                      <div className="flex items-center gap-1 text-slate-700">
                        ...
                        <span className="font-bold text-slate-900">{formatNumber(video.viewCount)}</span>
                      </div>

                      {/* Likes */}
                      <div className="flex items-center gap-1 text-slate-700">
                        <ThumbsUp className="w-3 h-3 text-slate-500" />
                        <span className="font-bold text-slate-900">{formatNumber(video.likeCount)}</span>
                      </div>

                      {/* Comments */}
                      <div className="flex items-center gap-1 text-slate-700">
                        <MessageSquare className="w-3 h-3 text-slate-500" />
                        <span className="font-bold text-slate-900">{formatNumber(video.commentCount)}</span>
                      </div>

                      {/* Engagement Rate */}
                      <div className="flex items-center gap-1 text-slate-700">
                        <span className="font-bold text-emerald-700">{typeof video.engagementRate === 'string' ? video.engagementRate : video.engagementRate?.toFixed(2) || '0'}%</span>
                      </div>
                    </div>

                    {/* Date */}
                    <div className="flex items-center gap-1.5 text-[9px] text-slate-500 border-t border-slate-200 pt-2 mt-auto">
                      <Calendar className="w-3 h-3 text-slate-400" />
                      <span className="text-slate-700 font-medium">{formatDate(video.publishedAt)}</span>
                    </div>
                  </div>
                </a>
              );
