...
import { Eye, ThumbsUp, MessageSquare, Calendar, Play, FileVideo } from 'lucide-react';
...
export const VideoGrid: React.FC<VideoGridProps> = ({ analysis }) => {
  const getVideoThumbnail = (videoId: string) => {
    if (!videoId || videoId === 'undefined') return 'https://via.placeholder.com/320x180?text=Video';
    return `https://img.youtube.com/vi/${videoId}/hqdefault.jpg`;
  };

  const getYoutubeUrl = (videoId: string) => {
    if (!videoId || videoId === 'undefined') {
      return 'https://www.youtube.com';  // Fallback URL
    }
      ...
      {analysis.map((company, companyIdx) => (
        <div key={`${company.companyName}-${companyIdx}`} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {company.videos.slice(0, 9).map((video, vidIdx) => {
              const uniqueKey = `${company.companyName}-${vidIdx}-${video.videoId || 'unknown'}`;
                ...
                >
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-3">
                      ...
                      {/* Engagement Rate */}
                      <div className="flex items-center gap-1.5 text-[11px] text-slate-400">
                        <span className="font-semibold text-emerald-400">{typeof video.engagementRate === 'string' ? video.engagementRate : video.engagementRate?.toFixed(2)}%</span>
