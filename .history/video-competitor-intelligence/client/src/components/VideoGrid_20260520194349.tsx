      ...
      {analysis.map((company, companyIdx) => (
        <div key={`${company.companyName}-${companyIdx}`} className="space-y-4">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className={`w-3 h-3 rounded-full ${company.isMainCompany ? 'bg-blue-600' : 'bg-purple-500'}`} />
            <h3 className="text-lg font-extrabold text-slate-900 tracking-tight">
              {company.companyName}
              {company.isMainCompany && <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full font-semibold border border-blue-200">Your Brand</span>}
            </h3>
          </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3 flex flex-col flex-1">
                    {/* Title */}
                    <h4 className="text-sm font-bold text-white line-clamp-2 group-hover:text-indigo-300 transition-colors mb-2">
                      {video.title}
                    </h4>
                    ...
                    <div className="grid grid-cols-2 gap-2 mb-2 text-[10px]">
                      ...
                      <div className="flex items-center gap-1 text-slate-300">
                        ...
                        <span className="font-bold text-white">{formatNumber(video.likeCount)}</span>
                      </div>

                      {/* Comments */}
                      <div className="flex items-center gap-1 text-slate-300">
                        <MessageSquare className="w-3 h-3 text-slate-400" />
                        <span className="font-bold text-white">{formatNumber(video.commentCount)}</span>
                      </div>

                      {/* Engagement Rate */}
                      <div className="flex items-center gap-1 text-slate-300">
                        <span className="font-bold text-emerald-300">{typeof video.engagementRate === 'string' ? video.engagementRate : video.engagementRate?.toFixed(2) || '0'}%</span>
                      </div>
                    </div>

                    {/* Date */}
                    <div className="flex items-center gap-1.5 text-[9px] text-slate-400 border-t border-white/10 pt-2 mt-auto">
                      <Calendar className="w-3 h-3 text-slate-500" />
                      <span className="text-slate-300 font-medium">{formatDate(video.publishedAt)}</span>
                    </div>
                  </div>
                </a>
              );
